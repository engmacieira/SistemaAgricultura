...
def test_obter_servico(mock_uc):
    ...
    response = client.get("/api/servicos/1")
    assert response.status_code == 200
...
def test_criar_servico(mock_uc):
    ...
    response = client.post("/api/servicos/", json={"name": "Aração", "description": "desc", "unit": "hr", "basePrice": 100.0, "active": True})
...
def test_atualizar_servico(mock_uc):
    ...
    response = client.put("/api/servicos/1", json={"name": "Novo"})
    assert response.status_code == 200

def test_deletar_servico(mock_uc):
    mock_uc.deletar_servico.return_value = True
    response = client.delete("/api/servicos/1")
