...
def test_criar_servico(client):
    ...
    response = client.post("/api/servicos/", json=payload)
...
def test_listar_servicos(client):
    ...
    client.post("/api/servicos/", json=payload)
...
def test_obter_servico(client):
    ...
    create_res = client.post("/api/servicos/", json=payload)
    servico_id = create_res.json()["id"]
    
    response = client.get(f"/api/servicos/{servico_id}")
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_atualizar_servico(client):
    ...
    response = client.put(f"/api/servicos/{servico_id}", json=update_payload)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_deletar_servico(client):
    ...
    response = client.delete(f"/api/servicos/{servico_id}")
    assert response.status_code == status.HTTP_204_NO_CONTENT
    
    get_res = client.get(f"/api/servicos/{servico_id}")
